...
export default function UserWallet() {
  ...
  const handleWalletConnect = async (connectedAddress) => {
    if (session?.user?.id) {
      try {
        ...
        console.log("User wallet: Wallet address saved", { address: connectedAddress });
      } catch (error) {
        console.error("User wallet: Error saving wallet address", error);
      }
    }
    setShowWalletPopup(false);
  };
  ...
  const handleDisconnect = () => {
    disconnect();
    console.log("User wallet: Wallet disconnected");
